...
class AccessLogProcessor(object):
    ...
    def process(self, obj_streams):
        '''generate hourly groupings of data from one access log file, 
            obj_streams = [
                (fd, data_object_account, data_object_container, data_object_name)]
        '''
        hourly_aggr_info = {}
        total_lines = 0
        bad_lines = 0
        for obj_stream in obj_streams:
            fd, data_object_account, data_object_container, data_object_name = obj_stream
            for line in fd:
                ...
                if not line_data:
                    bad_lines += 1
                    continue
                auth_token = line_data['auth_token']
                ...
                aggr_key = (auth_token, account, year, month, day, hour)
